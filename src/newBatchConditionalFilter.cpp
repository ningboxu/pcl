                       ...
                       const std::string& output_file)
{
    ...
    boost::filesystem::create_directories(
        output_file.substr(0, output_file.find_last_of('/')));
...
int main(int argc, char** argv)
{
    ...
    std::string new_base_directory = "/home/xnb/test/pcl/data/filtered_livox";
         ...
         iter != end; ++iter)
    {
        ...
        if (entry.path().extension() == ".pcd")
        {
            ...
            std::string relative_path =
                boost::filesystem::relative(input_path, base_directory)
                    .string();
            std::string output_path =
                new_base_directory + "/" +
                relative_path.substr(0, relative_path.size() - 4) +
                "_filtered.pcd";
